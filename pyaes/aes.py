...
__all__ = [
    "AES", "AESModeOfOperationCTR", "AESModeOfOperationCBC",
    "AESModeOfOperationCFB", "AESModeOfOperationECB", "AESModeOfOperationOFB",
    "AESModesOfOperation", "Counter"
]
...
class AESModeOfOperationCBC(AESBlockModeOfOperation):
    ...
    def encrypt(self, plaintext):
        ...
        precipherblock = [(p ^ l)
                          for (p, l) in zip(plaintext, self._last_cipherblock)]
    ...
    def decrypt(self, ciphertext):
        ...
        plaintext = [(p ^ l) for (
            p,
            l) in zip(self._aes.decrypt(cipherblock), self._last_cipherblock)]
...
class AESModeOfOperationCFB(AESSegmentModeOfOperation):
    ...
    def encrypt(self, plaintext):
        if len(plaintext) % self._segment_bytes != 0:
            raise ValueError(
                'plaintext block must be a multiple of segment_size')
        ...
        for i in xrange(0, len(plaintext), self._segment_bytes):
            ...
            xor_segment = self._aes.encrypt(
                self._shift_register)[:len(plaintext_segment)]
            cipher_segment = [
    ...
    def decrypt(self, ciphertext):
        if len(ciphertext) % self._segment_bytes != 0:
            raise ValueError(
                'ciphertext block must be a multiple of segment_size')
        ...
        for i in xrange(0, len(ciphertext), self._segment_bytes):
            ...
            xor_segment = self._aes.encrypt(
                self._shift_register)[:len(cipher_segment)]
            plaintext_segment = [
